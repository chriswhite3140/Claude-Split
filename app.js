 ...
 *
 * v1.16.3 - Fix Weekly Planner's Unit lessons rail wrongly disabling manual collapse
 *   for a subject-only filter. plannerUnitGroupIsCollapsed()/plannerUnitSidebarHtml()
 *   both force-expanded a group and disabled its collapse toggle whenever ANY rail
 *   filter was active (search OR subject). But only search can produce a partial match
 *   within a group (some lessons match, some don't) — the actual reason force-expand
 *   exists, to avoid hiding a match behind a collapsed heading. A subject filter alone
 *   is all-or-nothing per group (a unit's lessons all share one subject), so a group
 *   visible at all under a subject-only filter already has every lesson matching —
 *   nothing to protect by forcing it open. Renamed the shared check from
 *   plannerRailFilterActive() to plannerRailSearchActive(), now search-only; the
 *   subject filter's own matching logic (plannerRailFilteredGroups) is untouched.
 *   2 new/updated regression tests (313 total), confirmed to fail against the pre-fix
 *   code. Verified live (Test Mode sample data): a subject-only filter left manual
 *   collapse fully respected (toggle enabled, state preserved); adding a search term on
 *   top still force-expanded and disabled the toggle; clearing just the search term
 *   (subject filter left active) immediately restored manual collapse without needing
 *   to also clear the subject filter.
 ...
 *       focused heading button — added a data-unit-id attribute to the heading and a
 *       focus-restore step after the rebuild, same convention as
 *       plannerToggleICSuggestionGroup's existing data-group-key restore.
 *   (2) .planner-unit-group-count was a fixed 10px, smaller than the app's --label-size
 *       (13px/14px large-text) used by its own sibling heading text, and didn't scale
 *       under the large-text preference — switched to var(--label-size).
 *   3 new regression tests, all confirmed to fail against the pre-fix code; full suite
 *   254/254 passing.
...
function capturePanelScrollPositions(main) {
  ...
  PANEL_SCROLL_BODY_CLASSES.forEach(cls => {
    ...
    if (el) positions[cls] = el.scrollTop;
  });
  // Re-applying a captured scroll position only makes sense when the re-render is
  // redrawing the SAME logical content (e.g. the same open lesson's drawer, after
...
function renderPlanner(main) {
  ...
  const railCollapsed = state.plannerUi.railCollapsed;
  const drawerCollapsed = state.plannerUi.drawerCollapsed;
  ...
  main.innerHTML = `
    ...
    <div class="content planner-shell-layout" style="grid-template-columns: ${railCollapsed ? '40px' : 'minmax(212px, 252px)'} minmax(0, 1fr) ${drawerCollapsed ? '40px' : 'minmax(260px, 320px)'}">
      <aside class="card planner-unit-rail${railCollapsed ? ' is-collapsed' : ''}">
        ${railCollapsed ? `
          <button class="planner-panel-collapse-toggle" type="button" title="Expand Unit lessons" aria-label="Expand Unit lessons panel" onclick="plannerToggleRailCollapsed()">›</button>
        ` : `
          <div class="card-head">
            ...
            <div style="display:flex;align-items:center;gap:10px">
              ...
              <button class="planner-panel-collapse-toggle" type="button" title="Collapse" aria-label="Collapse Unit lessons panel" onclick="plannerToggleRailCollapsed()">‹</button>
            </div>
          </div>
          ${plannerRailFiltersHtml()}
          <div class="planner-unit-rail-body" id="planner-unit-rail-body">${plannerUnitSidebarHtml()}</div>
        `}
      </aside>
      ...
      <aside class="card planner-shell-drawer${drawerCollapsed ? ' is-collapsed' : ''}">
        ${drawerCollapsed ? `
          <button class="planner-panel-collapse-toggle" type="button" title="Expand Lesson Drawer" aria-label="Expand Lesson Drawer panel" onclick="plannerToggleDrawerCollapsed()">‹</button>
        ` : `
          <div class="card-head">
            ...
            <div style="display:flex;align-items:center;gap:10px">
              ...
              <button class="planner-panel-collapse-toggle" type="button" title="Collapse" aria-label="Collapse Lesson Drawer panel" onclick="plannerToggleDrawerCollapsed()">›</button>
            </div>
          </div>
...
function plannerUnitOccurrenceCardHtml(lesson, weekKey, dayKey) {
  ...
  // occurrence cards must be able to disagree. A single-occurrence lesson normally has
  // no per-slot taught flag in use, so it reads teachingStatus (the same thing the
  // checkbox itself sets for this case) — but it must ALSO still honor the slot's own
  // flag if one happens to be set (e.g. a lesson reduced from multi- to single-slot via
  // plannerUnscheduleSlot, which deliberately leaves teachingStatus untouched — see its
  // own comment — so a still-taught surviving occurrence must not read as unchecked
  // just because the lesson-wide status is now stale). unitLessonIsEffectivelyTaught
  // already encodes exactly this "either signal counts" rule, so it's reused here
  // rather than re-deriving the same logic inline. The status badge alongside it still
  // shows the lesson-wide picture (with its own taught/total fraction for a multi-slot
  // lesson).
  const isTaught = isMultiSlot ? !!(slot && slot.taught === true) : unitLessonIsEffectivelyTaught(lesson);
  ...
  // lesson there's no per-slot ambiguity, so it sets the lesson's overall teachingStatus
  // instead, via unitSetSingleOccurrenceTaught — the same underlying
  // unitSetLessonTeachingStatus the drawer's "Teaching status" dropdown already uses
  // for that case, plus reconciling this slot's own flag to match (see that function's
  // own comment for why that reconciliation matters). Neither path adds any gating
  // beyond what unitSetLessonTeachingStatus/unitToggleOccurrenceTaught already enforce
  // today (currently neither has an IC-linked gate, unlike the standalone equivalent —
  // see plannerSetLessonStatus).
  const taughtToggleExpr = isMultiSlot
    ...
    : `unitSetSingleOccurrenceTaught('${plannerJsStr(lesson.id)}','${plannerJsStr(weekKey)}','${plannerJsStr(dayKey)}',this.checked)`;
  const openExpr = `plannerOpenLessonDrawerFromCard('${plannerJsStr(lesson.id)}')`;
  return `
...
function plannerRailFiltersHtml() {
  return `
    <div class="planner-unit-rail-filters">
      <input class="form-input" type="text" placeholder="Search lessons" aria-label="Search unit lessons by title"
        value="${escapeHtml(state.plannerUi.railSearch || '')}" oninput="plannerRailHandleSearchInput(this.value)">
      <div style="display:flex;gap:6px;align-items:center">
        <select class="form-input" aria-label="Filter unit lessons by subject" style="flex:1" onchange="plannerRailHandleSubjectFilter(this.value)">
          <option value="">All subjects</option>
          ${PLANNER_SUBJECTS.map(s => `<option value="${s}" ${state.plannerUi.railSubjectFilter === s ? 'selected' : ''}>${s}</option>`).join('')}
        </select>
        <button class="btn" type="button" title="Clear filters" onclick="plannerRailClearFilters()">✕</button>
      </div>
    </div>
  `;
}

// Targeted refresh used by the search input's oninput so typing doesn't lose focus —
// same pattern as plannerHandleICSearchInput/unitHandleCDSearch: only the rail body
// is replaced, not the whole view.
function plannerRailRefreshBody() {
  const body = document.getElementById('planner-unit-rail-body');
  if (body) body.innerHTML = plannerUnitSidebarHtml();
}

function plannerRailHandleSearchInput(value) {
  plannerEnsureUiState();
  state.plannerUi.railSearch = value;
  plannerRailRefreshBody();
}

function plannerRailHandleSubjectFilter(value) {
  plannerEnsureUiState();
  state.plannerUi.railSubjectFilter = value;
  plannerRailRefreshBody();
}

function plannerRailClearFilters() {
  plannerEnsureUiState();
  state.plannerUi.railSearch = '';
  state.plannerUi.railSubjectFilter = '';
  renderView();
}

// True while the rail's search box specifically has a term in it — used to decide
// whether a unit group's manual collapse state should be overridden at render time
// (see plannerUnitGroupIsCollapsed). Only search can produce a PARTIAL match within a
// group (some of a unit's lessons match, some don't) — that's the actual reason the
// force-expand/disable-toggle behavior exists, to avoid hiding a match behind a
// collapsed heading. The subject filter alone is all-or-nothing per group (a unit's
// lessons all share one subject — see plannerRailFilteredGroups), so a group that's
// visible at all under a subject-only filter already has every one of its lessons
// matching; there's nothing left to protect by forcing it open. Deliberately does NOT
// look at railSubjectFilter (review finding: it previously did, forcing every group
// open — and disabling manual collapse — for a subject-only filter that had nothing to
// hide).
...
// (false/absent) so first load never opens with everything closed. While a search term
// is active this always returns false regardless of the stored value: plannerRailFilteredGroups
...
// stored value itself is left untouched by this override, so once the search clears,
// plannerUnitSidebarHtml goes back to whatever the teacher had manually set —
// collapse state and search state cooperate instead of one clobbering the other. A
// subject filter alone does not trigger this override — see plannerRailSearchActive.
function plannerUnitGroupIsCollapsed(unitId) {
  if (plannerRailSearchActive()) return false;
...
function plannerToggleUnitGroupCollapsed(unitId) {
  ...
  plannerRailRefreshBody();
  // The innerHTML rebuild above removes the focused heading button from the DOM — a
  // keyboard user activating it with Enter/Space would otherwise lose their position
  // after every expand/collapse. Re-focus its replacement (same convention as
  // plannerToggleICSuggestionGroup's data-group-key restore, review finding on this PR).
  const container = document.getElementById('planner-unit-rail-body');
  const sel = (window.CSS && CSS.escape) ? CSS.escape(unitId) : unitId;
  const heading = container && container.querySelector('[data-unit-id="' + sel + '"]');
...
function plannerUnitSidebarHtml() {
  ...
  const groups = plannerRailFilteredGroups(allGroups);
  if (!groups.length) {
    return `<div class="planner-unit-rail-empty">No unit lessons match your search or filter.</div>`;
  }
  const searchActive = plannerRailSearchActive();
  return groups.map(({ unit, lessons }) => {
    ...
    const toggleAttrs = searchActive
...
function plannerICResultsHtml(lesson) {
  ...
  // scores is keyed by IC id directly (plannerSuggestICsFromIntention scores each
  // candidate IC against its own text) — no more deriving a score via the IC's home/
  // linked descriptors. Normalise against the ICs that actually render here.
  const maxScore = showConfidence
    ? Math.max(1, ...resultIcs.map(ic => scores[ic.id] || 0))
    : 0;
  ...
  // Suggestion path: four collapsible groups — Strong vs Other confidence (a simpler
  // two-way split than the strong/partial/weak tiers used elsewhere, just for this
  // grouping), each further split by whether the IC belongs to this unit's linked CDs
  // (same icBelongsToCDs/linkedCDs check as the "From this unit's CDs" browse group
  // above). Order: strong+linked, strong+other, other+linked, other+other. A lesson
  // with no unit (or a unit with no linkedCDIds) has linkedCDs === null, so every IC's
  // "linked" test is always false — the two "…linked" groups stay permanently empty
  // (and hidden), which naturally collapses this to a plain Strong/Other split without
  // a separate branch.
  const withConf = resultIcs.map(ic => {
    const score = scores[ic.id] || 0;
    const conf = plannerConfidenceTier(score, maxScore);
    return {
      ic, score, conf,
      isStrong: !!conf && conf.key === 'strong',
      isLinked: !!linkedCDs && icBelongsToCDs(ic, linkedCDs),
    };
  });
  const byScoreDesc = (a, b) => b.score - a.score;
  const bucket = (isStrong, isLinked) => withConf.filter(x => x.isStrong === isStrong && x.isLinked === isLinked).sort(byScoreDesc);

  // Defaults: the lead group (this unit's strong matches, or — with no linked-CD
  // distinction to make — just the strong matches) opens by default; every other
  // group starts collapsed.
  const defaultOpen = {
    strongLinked: true,
    strongOther: !linkedCDs,
    otherLinked: false,
    otherOther: false,
  };
  const groups = [
    { key: 'strongLinked', items: bucket(true, true), label: "Strong matches - this unit's CDs" },
    { key: 'strongOther', items: bucket(true, false), label: linkedCDs ? 'Strong matches - other CDs' : 'Strong matches' },
    { key: 'otherLinked', items: bucket(false, true), label: "Other matches - this unit's CDs" },
    { key: 'otherOther', items: bucket(false, false), label: linkedCDs ? 'Other matches - other CDs' : 'Other matches' },
  ];

  return groups.map(({ key, items, label }) => {
    if (!items.length) return ''; // a group with zero results is hidden entirely, not shown empty
    ...
    return `
      <div class="planner-ic-suggestion-group ${open ? 'is-open' : ''}">
        ...
        ${open ? items.map(x => rowHtml(x.ic, x.conf)).join('') : ''}
      </div>
    `;
  }).join('') + createRow;
}
...
// A suggestion-path group (see plannerICResultsHtml) is open if explicitly toggled,
// else falls back to its default for this render (the lead group open, others
// collapsed — see defaultOpen there). Explicit toggles are keyed by group key alone,
// not by lesson/result set, so they're reset back to defaults on every fresh
// plannerSuggestICsFromIntention() run rather than persisting across unrelated
// suggestions.
function plannerIsICSuggestionGroupOpen(key, defaultOpen) {
  const stored = state.plannerUi?.icSuggestionGroupsOpen?.[key];
  return typeof stored === 'boolean' ? stored : !!defaultOpen;
}

// Re-render only the results container (same convention as plannerToggleICExpand)
// so the search field's focus isn't disturbed by an innerHTML rebuild.
function plannerToggleICSuggestionGroup(key, defaultOpen) {
  plannerEnsureUiState();
  const current = plannerIsICSuggestionGroupOpen(key, defaultOpen);
  state.plannerUi.icSuggestionGroupsOpen[key] = !current;
  const lesson = state.lessonPlans.find(item => item.id === state.plannerUi.selectedLessonId);
  const container = document.getElementById('planner-ic-results');
  if (!lesson || !container) return;
  const scrollTop = container.scrollTop;
...
// intention length, so absolute cut-offs don't generalise on their own). Boundaries
// from the observed distribution: top cluster >=0.80 strong, mid 0.50-0.79 partial,
// tail <0.50 weak. The ratio alone isn't enough, though: if the whole candidate pool
// scores low, the best-of-a-bad-bunch would still read >=0.80 and get called "Strong".
// PLANNER_MIN_STRONG_SCORE is an absolute floor on top of the ratio — a match must
// clear both to be "Strong", so a weak pool's least-bad match falls to "Partial"/"Weak"
// instead of misleadingly reading as a confident match.
function plannerConfidenceTier(score, maxScore) {
  if (!score || !maxScore) return null;
  const ratio = score / maxScore;
  if (ratio >= 0.80 && score >= PLANNER_MIN_STRONG_SCORE) return { key: 'strong', label: 'Strong' };
...
function plannerEnsureUiState() {
  ...
  if (!state.plannerUi.icSuggestionGroupsOpen || typeof state.plannerUi.icSuggestionGroupsOpen !== 'object') state.plannerUi.icSuggestionGroupsOpen = {};
  // Collapse state for the Unit lessons rail / Lesson Drawer side panels — session-only
  // (not persisted, not saved to localStorage/Sheets), so every fresh load starts with
  // both panels expanded regardless of how they were left last time.
  if (typeof state.plannerUi.railCollapsed !== 'boolean') state.plannerUi.railCollapsed = false;
  if (typeof state.plannerUi.drawerCollapsed !== 'boolean') state.plannerUi.drawerCollapsed = false;
  // Unit lessons rail search/subject filter — UI-only, never touches state.lessonPlans/
  // state.unitPlans, purely controls what plannerUnitSidebarHtml shows (see
...
function plannerToggleRailCollapsed() {
  plannerEnsureUiState();
  state.plannerUi.railCollapsed = !state.plannerUi.railCollapsed;
  renderView();
}

function plannerToggleDrawerCollapsed() {
  plannerEnsureUiState();
  state.plannerUi.drawerCollapsed = !state.plannerUi.drawerCollapsed;
  renderView();
}
...
// The actual calendar date (ISO) of one occurrence — weekKey is the week's Monday,
// dayKey is which weekday within it. Used by the Daily Log Wizard auto-launch (see
// dlLaunchOrMergeForLesson) to date a session to the day a lesson was actually taught,
// not always today, since a lesson can be marked taught after the fact. Falls back to
// today for a malformed/legacy weekKey or dayKey (e.g. the legacy 'unscheduled' value)
// rather than throwing.
function plannerDateForSlot(weekKey, dayKey) {
  const offset = PLANNER_SCHEDULABLE_DAYS.indexOf(dayKey);
  if (offset < 0 || !isValidIsoDate(weekKey)) return toIsoDate(new Date());
  return addDaysToDate(weekKey, offset);
}
...
// score each candidate IC directly against its own text, then surface the
// best-matching ICs for the teacher to tick.
...
// to the rest of the candidate pool — a single low-weight token overlap is usually
// coincidental, not real relevance.
const PLANNER_MIN_SUGGESTION_SCORE = 2;
// A match below this raw score can never be tagged "Strong", even if it's the best (or
// only) match in an otherwise-weak pool — plannerConfidenceTier()'s existing ratio
// check is a secondary signal on top of this floor, not a replacement for it. Both
// floors were derived by scoring the real curriculum ICs bundled in data/ against a
// spread of real learning intentions and inspecting the resulting score distributions
// (see the PR description for the worked examples) — not guessed.
const PLANNER_MIN_STRONG_SCORE = 3;

// Same >=4-char minimum as before, plus curriculum-vocabulary stopword removal —
// shared by the intention text and (via plannerScoreIC) each candidate IC's own text,
// so both sides of a match are cleaned identically.
function plannerCleanTokens(text) {
  return [...new Set(
    (text || '').toLowerCase().replace(/[^a-z0-9\s]/g, ' ').split(/\s+/)
      .filter(w => w.length >= 4 && !PLANNER_SUGGESTION_STOPWORDS.has(w))
  )].slice(0, 25);
}

// Score a candidate IC directly against its OWN text (name, description,
// exampleOfSuccess) — far more specific than its parent descriptor's text, and the
// whole point of scoring at this level: ICs under one broad descriptor can vary
// hugely in actual content (e.g. an addition-strategy IC and a halving-for-
// multiplication IC homed on the same "addition and subtraction, and multiplication"
// descriptor), so they must no longer all inherit an identical descriptor-level score.
function plannerScoreIC(ic, tokens) {
  const text = [ic.name || '', ic.description || '', ic.exampleOfSuccess || ''].join(' ').toLowerCase();
  let score = 0;
...
function plannerSuggestICsFromIntention() {
  ...
  const tokens = plannerCleanTokens(intention);
  ...
  // For a unit lesson, ICs belonging to the unit's own linked CDs (unit.linkedCDIds)
  // should surface before any other matching IC, regardless of token score — a
  // priority boost, not a restriction. ICs outside the unit's CDs are still scored
  // and shown below, same as today. null for standalone lessons and units with no
  // linkedCDIds set yet, leaving those cases' ordering unchanged.
  const unit = unitForLesson(lesson);
  const unitCDIds = unit && Array.isArray(unit.linkedCDIds) && unit.linkedCDIds.length
    ? new Set(unit.linkedCDIds) : null;
  ...
  const suggested = ranked.map(r => r.ic.id);
  ...
  state.plannerUi.icSuggestionGroupsOpen = {};
...
function unitPlansEnsureUiState() {
  ...
  // Unit Plans list-view search/subject filter — UI-only, never touches state.unitPlans
  // itself, purely controls what renderUnitList shows (see unitListFilteredUnits).
  if (typeof state.unitPlansUi.listSearch !== 'string') state.unitPlansUi.listSearch = '';
  if (typeof state.unitPlansUi.listSubjectFilter !== 'string') state.unitPlansUi.listSubjectFilter = '';
...
// unitLessonDerivedTeachingStatus). Only called for a lesson with more than one
// scheduled occurrence (see plannerUnitOccurrenceCardHtml / unitLessonScheduleHtml) —
// a single-occurrence lesson has no per-occurrence ambiguity to resolve, so it's set
// taught/not-taught a different way (see unitSetSingleOccurrenceTaught below).
function unitToggleOccurrenceTaught(lessonId, weekKey, dayKey) {
  ...
  if (slotIdx < 0) return;
  const isMultiSlot = slots.length > 1; // captured before mutation, for the wizard-trigger guard below
  ...
  state.lessonPlans[idx] = nextLesson;
  saveLessonPlansState();
  // Only trigger the Daily Log Wizard from here for a genuine multi-slot occurrence —
  // the multi-slot board checkbox's own direct call target (isMultiSlot captured
  // above, before this slot's own flag was mutated). unitSetSingleOccurrenceTaught
  // also calls this function internally to reconcile a single slot's own stale flag;
  // that path already triggers the wizard itself with lesson-wide semantics, so it
  // must not double-fire here too.
  if (nowTaught && isMultiSlot) dlLaunchOrMergeForLesson(nextLesson, weekKey, dayKey);
  renderView();
}

// The Week Board checkbox's write path for a single-occurrence unit lesson's card
// (plannerUnitOccurrenceCardHtml). Sets teachingStatus via the same function the
// drawer's "Teaching status" dropdown already uses (unitSetLessonTeachingStatus), then
// reconciles this one slot's own taught flag to match — without this second step, a
// lesson that still carries a stale per-occurrence taught flag from before it was
// reduced to one slot (plannerUnscheduleSlot deliberately leaves teachingStatus
// untouched when a slot is removed, by design — see its own comment) could keep
// reading as taught via unitLessonIsEffectivelyTaught even after being unchecked here,
// since nothing would otherwise ever clear that leftover flag. Reuses
// unitToggleOccurrenceTaught itself to touch the slot (its own re-derive call is a
// no-op for a single-slot lesson, so it never fights the teachingStatus already just
// set above) rather than duplicating slot-mutation logic here.
function unitSetSingleOccurrenceTaught(lessonId, weekKey, dayKey, taught) {
  unitSetLessonTeachingStatus(taught ? 'taught' : 'planned', lessonId);
  const lesson = state.lessonPlans.find(l => l.id === lessonId);
  if (!lesson) return;
  const slots = Array.isArray(lesson.scheduledSlots) ? lesson.scheduledSlots : [];
  const slot = slots.find(s => s && s.weekKey === weekKey && s.dayKey === dayKey);
  if (!!(slot && slot.taught === true) !== taught) {
    unitToggleOccurrenceTaught(lessonId, weekKey, dayKey);
  }
  if (taught) dlLaunchOrMergeForLesson(lesson, weekKey, dayKey);
}
...
// ── LIST VIEW ──
// Search matches unit title (case-insensitive substring); subject filter is an exact
// match against unit.subject. Both combine with AND. UI-only — never touches
// state.unitPlans, purely controls what unitListBodyHtml/renderUnitList show.
function unitListFilteredUnits(units) {
  const search = (state.unitPlansUi.listSearch || '').trim().toLowerCase();
  const subject = state.unitPlansUi.listSubjectFilter || '';
  return units.filter(unit => {
    if (subject && unit.subject !== subject) return false;
    if (search && !(unit.title || '').toLowerCase().includes(search)) return false;
    return true;
  });
}

// The #unit-list-body content: the unit grid, the "no units yet" empty state, or —
// when a search/subject filter combination matches nothing — a distinct "no units
// match" message, so a filtered-to-empty list doesn't look like a broken empty list.
function unitListBodyHtml(units) {
  if (units.length === 0) {
  ...
  const filtered = unitListFilteredUnits(units);
  if (filtered.length === 0) {
    return `<div class="card" style="padding:32px;text-align:center">
             <div style="font-size:15px;font-weight:600;margin-bottom:6px">No units match</div>
             <div style="font-size:13px;color:var(--text3)">Try a different search term or subject filter.</div>
           </div>`;
  }
  const cards = filtered.map(unit => {
    const stats = unitLessonStats(unit);
  ...
  return `<div class="unit-card-grid">${cards}</div>`;
}

function unitListSortedUnits() {
  return [...state.unitPlans].sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt)));
}

// Targeted refresh used by the search input's oninput so typing doesn't lose focus —
// same pattern as unitHandleCDSearch/plannerHandleICSearchInput: only the results
// container is replaced, not the whole view.
function unitListRefreshBody() {
  const body = document.getElementById('unit-list-body');
  if (body) body.innerHTML = unitListBodyHtml(unitListSortedUnits());
}

function unitListHandleSearchInput(value) {
  unitPlansEnsureUiState();
  state.unitPlansUi.listSearch = value;
  unitListRefreshBody();
}

function unitListHandleSubjectFilter(value) {
  unitPlansEnsureUiState();
  state.unitPlansUi.listSubjectFilter = value;
  unitListRefreshBody();
}

function unitListClearFilters() {
  unitPlansEnsureUiState();
  state.unitPlansUi.listSearch = '';
  state.unitPlansUi.listSubjectFilter = '';
  renderView();
}

function renderUnitList(main) {
  unitPlansEnsureUiState();
  const units = unitListSortedUnits();
  ...
  main.innerHTML = `
    <div class="topbar" style="padding:14px 24px">
      ...
      <div class="topbar-actions" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
        ${units.length ? `
        <input class="form-input unit-list-search-input" type="text" placeholder="Search units by title" aria-label="Search units by title"
          value="${escapeHtml(state.unitPlansUi.listSearch || '')}" oninput="unitListHandleSearchInput(this.value)">
        <select class="form-input unit-field-sm" aria-label="Filter units by subject" onchange="unitListHandleSubjectFilter(this.value)">
          <option value="">All subjects</option>
          ${PLANNER_SUBJECTS.map(s => `<option value="${s}" ${state.unitPlansUi.listSubjectFilter === s ? 'selected' : ''}>${s}</option>`).join('')}
        </select>
        <button class="btn" type="button" onclick="unitListClearFilters()">Clear filters</button>
        ` : ''}
        <button class="btn btn-primary" type="button" onclick="unitCreateNew()">+ New Unit</button>
      </div>
    </div>
    <div class="content" id="unit-list-body">
      ${unitListBodyHtml(units)}
    </div>
  `;
}
...
let dlState = {
  ...
  icLessonMap: {},     // key: icId → lessonId, set only by the Week Board "mark as
                        // taught" auto-launch/merge (see dlLaunchOrMergeForLesson);
                        // an IC picked by hand has no entry — read by saveDailyLog to
                        // tag each written record with which lesson it came from
  isOpen: false,        // whether the modal is currently shown — set by
                        // openDailyLogWizard/closeDlModal, read by
                        // dlLaunchOrMergeForLesson to decide launch vs merge
};
...
// Bridges the Week Board "mark as taught" checkbox to the Daily Log Wizard (phase 1
// of closing the "Daily Wizard is isolated from planned lessons" gap — see CLAUDE.md).
// Called as a side effect after a checkbox's own status-mutation function (see
// plannerSetLessonStatus / unitToggleOccurrenceTaught / unitSetSingleOccurrenceTaught)
// has already succeeded and settled the lesson as taught — never in place of it, and
// never on unchecking (each call site only calls this on the taught transition).
// No-ops for a lesson with no linked ICs — there's nothing to log. If no wizard
// session is open, launches one pre-filled with this lesson's ICs and dated to the
// actual occurrence (weekKey/dayKey), not always today, since a lesson can be marked
// taught after the fact. If a session is already open (e.g. a second lesson checked
// on the board before the first session was saved/closed), merges this lesson's ICs
// into it instead of discarding the in-progress session or opening a second modal —
// dedupes any IC already present. dlState.icLessonMap records, per IC id, which
// lesson first contributed it (first-attribution-wins for the rare case of two
// lessons sharing the same IC) — read at save time by saveDailyLog to tag every
// written record with its lessonId.
function dlLaunchOrMergeForLesson(lesson, weekKey, dayKey) {
  const icIds = Array.isArray(lesson?.linkedICIds) ? lesson.linkedICIds.filter(Boolean) : [];
  if (!icIds.length) return;
  if (!dlState.isOpen) {
    openDailyLogWizard();
    dlState.date = plannerDateForSlot(weekKey, dayKey);
  }
  dlState.icLessonMap = dlState.icLessonMap || {};
  icIds.forEach(icId => {
    if (!dlState.selectedICs.includes(icId)) dlState.selectedICs.push(icId);
    if (!dlState.icLessonMap[icId]) dlState.icLessonMap[icId] = lesson.id;
  });
...
// A CD code entry has no single IC of its own — dlState.selectedCodes is a deduped
// union of manually-picked codes and every selected IC's homeDescriptorId (see
// dlRecalcSelectedCodes), so more than one IC (possibly from different lessons) can
// legitimately share a code. First IC found (in sessionICs' own order) that traces to
// a lesson wins the attribution for that code; a manually-picked code with no
// contributing IC stays untagged (lessonId null). This is the best traceability
// available for a code-level record — the IC-level records saveDailyLog also writes
// are always exact, one lessonId per IC, no attribution ambiguity. Pure function
// (no state reads beyond instructionalComponents/its own params) so it's directly
// testable without touching saveDailyLog's async apiCall/fetch path.
function dlDeriveCodeLessonIds(sessionICs, icLessonMap) {
  const codeToLessonId = {};
  sessionICs.forEach(icId => {
    const homeDescriptorId = state.instructionalComponents.find(ic => ic.id === icId)?.homeDescriptorId;
    if (!homeDescriptorId || codeToLessonId[homeDescriptorId]) return;
    const lessonId = icLessonMap[icId];
    if (lessonId) codeToLessonId[homeDescriptorId] = lessonId;
  });
  return codeToLessonId;
}
...
async function saveDailyLog() {
  ...
  // Which lesson (if any) first contributed each IC — populated only by the Week
  // Board "mark as taught" auto-launch/merge (see dlLaunchOrMergeForLesson); an IC
  ...
  const codeToLessonId = dlDeriveCodeLessonIds(sessionICs, icLessonMapSnapshot);
  ...
  try {
    // NOTE: Apps Script saveTaughtLog must accept and persist a lessonId field (a
    // column on the TaughtLog sheet) for this to actually reach Sheets — until that
    // update, it's silently dropped server-side while still landing in local state
    // below, same class of gap as the got_it/needs_review note further down.
  ...
  // NOTE: Apps Script saveTaughtICs must also accept and persist a lessonId field (a
  //       column on the TaughtICs sheet) for this to reach Sheets — same gap as above.
  // Each IC's own lessonId (exact, unlike the code-level best-effort attribution
  // above) — null for an IC added by hand rather than via the Week Board checkbox.
  const icEntries = [];
