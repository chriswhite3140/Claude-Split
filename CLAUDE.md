...
- **Planning data (units + lessons) is still localStorage-first, not Sheets-backed.** The Drive JSON backup (deployed and confirmed working — see Backend above) is a safety net against data loss, not a substitute — it's not queryable, and outcome data still can't be joined to it (a consequence of the `lessonId` gap above).
...
**A note on verifying Drive backup deployment status specifically:** this repo has no direct access to the separate Apps Script project behind `API_URL`, so a static claim here (or a comment in `apps-script/DriveBackup.gs`) can go stale the moment someone redeploys it outside the repo — as happened once already (an earlier version of this file wrongly claimed it wasn't deployed, based on a since-stale comment in that file). The live, checkable signal is the app's own Drive sync indicator (`driveSyncIndicatorHtml()` in `app.js`, shown in the Weekly Planner topbar, Unit Plans, and Data & Settings): "Last synced to Drive: `<time>`" means the backend is deployed and responding; "Drive sync failed — retry" (after clicking retry) or a claim from a code comment alone does not confirm either way — check the running app or ask the repo owner.
