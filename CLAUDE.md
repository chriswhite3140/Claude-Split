...
**A note on Progress write reliability (found + fixed 2026-08-23):** `updateProgress` could return `{success:true}` while silently changing nothing, because its column lookups (`headers.indexOf("mastery")`, `"date"`, etc.) assumed short header names that didn't match the live sheet's actual header row (`mastery_level`, `date_assessed`, etc.) — a mismatched `indexOf` silently returns `-1` and the write for that field is just skipped, with no error anywhere. `saveProgress`'s append path was never affected (it writes positionally, not by header name), which is why creating new records always worked while editing existing ones silently no-opped the whole time this went unnoticed. Multiple rounds of `app.js` client-side fixes (duplicate-row selection, backdate handling, re-entrant-save guards — all real bugs, all correctly fixed, none of them the actual cause) came before this was found, because the client had no way to see that a genuine `{success:true}` backend response wasn't truthful. Fixed directly in the live Apps Script project, not in this repo — `apps-script/` only contains `DriveBackup.gs`, so neither the bug nor the fix is visible from source here; if a similar "success reported but data unchanged" symptom ever resurfaces, check the relevant sheet's actual header row text before re-investigating the client. Two related things were flagged during that review but NOT verified/fixed: `updateStandardsJudgment`/`updateProgressionPlacement` use the identical short-name `indexOf` lookup pattern against `StandardsJudgments`/`ProgressionPlacements` (whether those sheets' real headers match is unconfirmed), and both functions still have the same `||`-falsy-fallback bug `updateProgress` was separately fixed for (explicitly clearing a field to empty silently keeps the old value instead).
